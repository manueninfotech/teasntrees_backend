    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex gap-4 p-1.5 bg-gray-50 rounded-2xl w-fit">
                <button onClick={() => { setReviewType('food'); setPage(1); }} className={`px-8 py-3 rounded-[1.25rem] text-[10px] font-black uppercase tracking-widest transition-all ${reviewType === 'food' ? 'bg-white text-bakery-primary shadow-sm border border-bakery-bg' : 'text-gray-400 hover:text-gray-600'}`}>Food Items</button>
                <button onClick={() => { setReviewType('site'); setPage(1); }} className={`px-8 py-3 rounded-[1.25rem] text-[10px] font-black uppercase tracking-widest transition-all ${reviewType === 'site' ? 'bg-white text-bakery-primary shadow-sm border border-bakery-bg' : 'text-gray-400 hover:text-gray-600'}`}>Cafe Experience</button>
                <button onClick={() => { setReviewType('rider'); setPage(1); }} className={`px-8 py-3 rounded-[1.25rem] text-[10px] font-black uppercase tracking-widest transition-all ${reviewType === 'rider' ? 'bg-white text-bakery-primary shadow-sm border border-bakery-bg' : 'text-gray-400 hover:text-gray-600'}`}>Rider Service</button>
            </div>
            ...
            <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden min-h-[500px] flex flex-col">
                <div className="flex border-b border-gray-100 bg-gray-50/50">
                    {['all', 'pending', 'approved'].map((tab) => (
                        <button key={tab} onClick={() => { setActiveTab(tab); setPage(1); }} className={`flex-1 py-5 text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'text-bakery-primary bg-white border-b-2 border-bakery-primary' : 'text-gray-400 hover:text-gray-600'}`}>{tab}</button>
                    ))}
                </div>
                ...
                <div className="p-8 flex-1">
                    {reviews.length === 0 ? (
                        ...
                        <div className="space-y-6">
                            {reviews.map((review) => (
                                <div key={review._id} className="group border-2 border-gray-50 rounded-[2rem] p-6 hover:border-bakery-light hover:bg-bakery-bg transition-all flex flex-col md:flex-row gap-6">
                                    <div className="w-16 h-16 bg-gray-50 rounded-[1.5rem] flex items-center justify-center group-hover:bg-white transition-all shadow-sm shrink-0">
                                        <span className="text-xl font-black text-bakery-primary uppercase">{review.customerId?.name?.charAt(0) || 'U'}</span>
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <div className="flex justify-between items-start mb-2">
                                            <div>
                                                ...
                                                <p className="text-[10px] font-black text-gray-400 uppercase tracking-tighter">
                                                    {review.type === 'site' || !review.orderId ? (
                                                        <span className="text-bakery-primary">STAY & DINE • SITE REVIEW</span>
                                                    ) : (
                                            ...
                                            <span className={`px-2 py-1 rounded-lg text-[8px] font-black uppercase tracking-widest ${review.isApproved ? 'bg-bakery-light text-bakery-primary' : 'bg-orange-100 text-orange-700'}`}>{review.isApproved ? 'Approved' : 'Pending'}</span>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            {!review.isApproved ? (
                                                <button onClick={() => handleApprove(review._id)} className="px-5 py-2.5 bg-bakery-bg text-bakery-primary rounded-xl text-[10px] font-black uppercase hover:bg-bakery-primary hover:text-white transition-all">Approve</button>
                                            ) : (
...
const StatCard = ({ label, value, icon: Icon, theme, desc, loading }) => {
    const themes = {
        bakery: 'from-bakery-primary to-bakery-accent shadow-bakery-light text-bakery-primary bg-bakery-bg',
        ...
        green: 'from-bakery-primary to-bakery-accent shadow-bakery-light text-bakery-primary bg-bakery-bg',
