...
export default function MessagesPage() {
    ...
    const getStatusColor = (status) => {
        switch (status) {
            ...
            case 'Resolved': return 'bg-bakery-light text-bakery-primary border-bakery-accent';
                        ...
                        {data?.data?.map((message) => (
                            ...
                            >
                                <div className={`absolute top-0 left-0 w-2 h-full ${message.status === 'New' ? 'bg-blue-500' :
                                        message.status === 'Resolved' ? 'bg-bakery-bg' : 'bg-yellow-500'
                                    }`} />
                                    ...
                                    <div className="lg:w-48 flex flex-row lg:flex-col gap-3 justify-center border-t lg:border-t-0 lg:border-l border-gray-100 pt-4 lg:pt-0 pl-0 lg:pl-6">
                                        {message.status !== 'Resolved' && (
                                            <button
                                                ...
                                                className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-bakery-bg text-bakery-primary rounded-xl font-bold hover:bg-bakery-light transition-colors text-sm"
                                            >
