...
const UserDetailsModal = ({ user, isOpen, onClose }) => {
    ...
    const roleBadge = (role) => {
        switch (role) {
            ...
            default: return { icon: User, color: 'text-bakery-primary', bg: 'bg-bakery-light' };
        }
    };
    ...
    const getActionColor = (action) => {
        switch (action) {
            case 'create': return 'text-bakery-primary bg-bakery-light';
            case 'update': return 'text-blue-600 bg-blue-100';
            case 'delete': return 'text-red-600 bg-red-100';
            default: return 'text-bakery-primary bg-bakery-light';
        }
    };
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-bakery-primary/40 backdrop-blur-sm">
            <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden border border-gray-100 flex flex-col">
                ...
                <div className={`p-8 flex items-center justify-between border-b border-gray-50 z-10 ${user.role === 'admin' ? 'bg-purple-50' :
                    ...
                    <button
                        onClick={onClose}
                        className="p-4 bg-white/50 text-gray-400 rounded-2xl hover:bg-bakery-primary hover:text-white transition-all border border-black/5"
                    >
                        <X className="w-6 h-6" />
                    </button>
                </div>
                    ...
                    {loading ? (
                        ...
                        <div className="space-y-10">
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div className={`p-6 rounded-[2rem] flex items-center gap-4 border ${isActive ? 'bg-bakery-bg border-bakery-light' : 'bg-red-50/50 border-red-100'}`}>
                                    <div className={`p-3 rounded-xl shadow-sm bg-white ${isActive ? 'text-bakery-primary' : 'text-red-600'}`}>
                                    ...
                                    <div>
                                        ...
                                        <p className={`text-[10px] font-black uppercase tracking-widest ${isActive ? 'text-bakery-primary' : 'text-red-700'}`}>{isActive ? 'ACTIVE' : 'INACTIVE'}</p>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        {details.wishlist.slice(0, 4).map(item => (
                                            <div key={item._id} className="flex items-center gap-4 p-4 border border-gray-100 rounded-[1.5rem] bg-white hover:border-bakery-primary transition-all group">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-3 gap-4">
                                        {[
                                            ...
                                            { label: 'State', value: details?.isOnline ? 'ONLINE' : 'OFFLINE', color: details?.isOnline ? 'text-bakery-primary' : 'text-gray-400' }
                                        ].map((stat, i) => (
                            ...
                            {(user.role === 'admin' || user.role === 'manager') && (
                                <div className="space-y-4">
                                    <div className="flex items-center gap-3 px-2">
                                        <div className="p-2 bg-bakery-bg rounded-lg text-bakery-accent">
                                    ...
                                    <div className="space-y-3">
                                        {logsLoading ? (
                                            ...
                                            activityLogs.slice(0, 5).map((log) => (
                                                <div key={log._id} className="flex items-start gap-4 p-6 bg-white border border-gray-100 rounded-[2rem] hover:border-bakery-primary transition-all group">
