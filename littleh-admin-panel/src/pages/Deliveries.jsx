...
const Deliveries = () => {
    ...
    const getStatusColor = (status) => {
        switch (status) {
            ...
            case 'picked_up': return 'bg-bakery-bg text-bakery-primary border-bakery-light';
            case 'in_transit': return 'bg-orange-50 text-orange-700 border-orange-100';
            case 'delivered': return 'bg-bakery-bg text-bakery-primary border-bakery-light';
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-6">
                ...
                <StatCard label="Completed" value={stats?.completedDeliveries || 0} icon={CheckCircle2} theme="bakery" desc="Landed today" loading={statsLoading} />
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {filteredDeliveries.length === 0 ? (
                                    ...
                                    filteredDeliveries.map((delivery) => (
                                        <tr key={delivery._id} className="group hover:bg-bakery-bg transition-all duration-300">
                                            <td className="px-8 py-6">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-12 h-12 bg-gray-50 border border-gray-100 text-bakery-primary rounded-2xl flex items-center justify-center font-black group-hover:scale-110 transition-transform">#{delivery.orderId?.orderNumber?.slice(-4)}</div>
