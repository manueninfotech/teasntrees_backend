...
const Payouts = () => {
    ...
    return (
        <div className="space-y-8 relative">
            ...
            {notification && (
                <div className={`p-6 rounded-[2rem] border-2 flex items-center gap-4 animate-in slide-in-from-top duration-300 shadow-xl ${notification.type === 'success' ? 'bg-bakery-bg text-bakery-primary border-bakery-light' : 'bg-red-50 text-red-700 border-red-100'}`}>
                    <div className={`p-2 rounded-xl ${notification.type === 'success' ? 'bg-bakery-light' : 'bg-red-100'}`}>
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {stats.length === 0 ? (
                                    ...
                                    stats.map((rider) => (
                                        <tr key={rider.riderId} className="group hover:bg-bakery-bg transition-all duration-300">
                                            ...
                                            <td className="px-8 py-6">
                                                <span className="text-xl font-black text-bakery-primary">₹{rider.totalAmount.toLocaleString()}</span>
                                            </td>
                                            <td className="px-8 py-6">
                                            ...
                                            <td className="px-8 py-6 text-right">
                                                <button
                                                    ...
                                                    className="inline-flex items-center gap-2 px-8 py-4 bg-black text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all hover:bg-bakery-primary shadow-xl shadow-black/10 disabled:opacity-50 group/btn active:scale-95"
                                                >
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-[2rem] w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                        ...
                        <div className="p-8 space-y-6">
                            <div className="flex items-center gap-4 p-4 bg-bakery-bg rounded-2xl border border-bakery-light">
                                <div className="p-3 bg-white rounded-xl text-bakery-primary shadow-sm">
                                    <User className="w-6 h-6" />
                                </div>
                                <div>
                                    <p className="text-[10px] font-black text-bakery-accent uppercase tracking-widest">Beneficiary</p>
...
const StatCard = ({ label, value, icon: Icon, theme, desc, loading }) => {
    const themes = {
        ...
        indigo: 'from-indigo-600 to-indigo-700 shadow-bakery-light text-bakery-primary bg-bakery-bg',
