        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-3">
                    ...
                    >
                        <RefreshCw className={`w-5 h-5 ${isSyncing ? 'animate-spin text-bakery-primary' : 'text-gray-400'}`} />
                    </button>
            ...
            {selectedProducts.size > 0 && (
                <div className="bg-bakery-primary rounded-2xl p-4 flex items-center justify-between text-white shadow-xl">
                    ...
                    <div className="flex gap-2">
                        ...
                        <button onClick={executeBulkAction} disabled={!bulkAction} className="bg-white text-bakery-primary px-6 py-2 rounded-xl text-[10px] font-black uppercase disabled:opacity-50">Apply</button>
                    </div>
                </div>
            )}
                            ...
                            <div key={product._id} className={`group bg-white rounded-[2rem] shadow-sm border-2 overflow-hidden hover:shadow-2xl hover:-translate-y-1 transition-all ${selectedProducts.has(product._id) ? 'border-bakery-primary ring-4 ring-bakery-light' : 'border-gray-50'}`}>
                                <div className="h-56 bg-gray-50 flex items-center justify-center relative">
                                    ...
                                    <button onClick={() => toggleProductSelection(product._id)} className="absolute top-4 left-4 p-2 bg-white/90 rounded-xl shadow-lg">
                                        {selectedProducts.has(product._id) ? <CheckSquare className="w-5 h-5 text-bakery-primary" /> : <Square className="w-5 h-5 text-gray-200" />}
                                    </button>
                                    <button onClick={() => toggleAvailability(product._id, product.isAvailable)} className={`absolute top-4 right-4 p-2 rounded-xl shadow-lg backdrop-blur-md ${product.isAvailable ? 'bg-bakery-light/80 text-bakery-primary' : 'bg-red-100/80 text-red-600'}`}>
                                ...
                                <div className="p-6 space-y-4">
                                    ...
                                    <div className="flex gap-2 pt-2">
                                        ...
                                        <button onClick={() => { setEditingProduct(product); setShowModal(true); }} className="flex-1 bg-gray-50 text-gray-400 py-3 rounded-2xl text-[10px] font-black uppercase hover:bg-bakery-primary hover:text-white transition-all">Edit</button>
