        ...
        <div className="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl transition-all group p-6">
            ...
            <div className="flex items-start justify-between mb-6">
                ...
                <div className="flex flex-col gap-1.5 items-end">
                    <span className={`px-2 py-1 rounded-md text-[8px] font-black uppercase ${customer.isActive ? 'bg-bakery-bg text-bakery-primary' : 'bg-red-50 text-red-600'}`}>
            ...
            <div className="flex gap-2 pt-6">
                <button onClick={() => onViewDetails(customer)} className="flex-1 bg-gray-50 text-gray-400 py-3 rounded-2xl text-[10px] font-black uppercase hover:bg-bakery-primary hover:text-white transition-all">Details</button>
                <button
                    onClick={() => onToggleStatus(customer)}
                    className={`px-4 py-3 rounded-2xl transition-all ${customer.isActive ? 'bg-gray-50 text-gray-400 hover:bg-orange-500 hover:text-white' : 'bg-bakery-bg text-bakery-primary hover:bg-bakery-primary hover:text-white'}`}
                >
