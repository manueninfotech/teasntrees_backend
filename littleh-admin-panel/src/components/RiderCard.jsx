        ...
        <div className="bg-white rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl transition-all group">
            ...
            <div className="p-6 pb-0 flex items-start justify-between">
                <div className="flex items-center gap-4">
                    <div className="w-14 h-14 bg-gray-50 rounded-2xl flex items-center justify-center border border-gray-100 group-hover:bg-bakery-bg transition-colors">
                ...
                {!isPending && (
                    <div className="flex flex-col gap-1.5 items-end">
                        <span className={`px-2 py-0.5 rounded-md text-[8px] font-black uppercase ${rider.isOnline ? 'bg-bakery-bg text-bakery-primary' : 'bg-gray-50 text-gray-400'}`}>
            ...
            <div className="p-6 space-y-5">
                ...
                <div className="flex gap-2 pt-2">
                    {isPending ? (
                        <>
                            <button onClick={() => onViewDetails(rider)} className="flex-1 bg-gray-50 text-gray-400 py-3 rounded-2xl text-[10px] font-black uppercase hover:bg-bakery-primary hover:text-white transition-all">Details</button>
                            <button onClick={() => onApprove(rider)} className="flex-1 bg-bakery-bg text-bakery-primary py-3 rounded-2xl text-[10px] font-black uppercase hover:bg-bakery-primary hover:text-white transition-all">Approve</button>
                        ...
                        <>
                            <button onClick={() => onViewDetails(rider)} className="flex-1 bg-gray-50 text-gray-400 py-3 rounded-2xl text-[10px] font-black uppercase hover:bg-bakery-primary hover:text-white transition-all">View</button>
                            <button onClick={() => onToggleStatus(rider)} className="px-4 py-3 bg-gray-50 text-gray-400 rounded-2xl hover:bg-bakery-primary hover:text-white transition-all"><Power className="w-4 h-4" /></button>
