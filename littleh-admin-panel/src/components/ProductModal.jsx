                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-8">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        <div className="col-span-2 space-y-2">
                            ...
                            <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full bg-gray-50 border-none rounded-2xl py-4 px-6 text-sm font-black uppercase tracking-widest placeholder:text-gray-300 focus:ring-2 focus:ring-bakery-primary focus:bg-white transition-all" placeholder="E.G. CLASSIC LATTE" required />
                        </div>

                        <div className="col-span-2 space-y-2">
                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Description *</label>
                            <textarea value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} className="w-full bg-gray-50 border-none rounded-2xl py-4 px-6 text-sm font-black uppercase tracking-widest placeholder:text-gray-300 focus:ring-2 focus:ring-bakery-primary focus:bg-white transition-all min-h-[100px] resize-none" placeholder="PRODUCT COMPOSITION AND TASTE NOTES..." required />
                        </div>
                        ...
                        {!isLittlehCakeCategory && (
                            <div className="space-y-2">
                                ...
                                <input type="number" step="0.01" value={formData.price} onChange={(e) => setFormData({ ...formData, price: e.target.value })} className="w-full bg-gray-50 border-none rounded-2xl py-4 px-6 text-sm font-black uppercase tracking-widest focus:ring-2 focus:ring-bakery-primary focus:bg-white transition-all" required />
                            </div>
                        )}

                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Category *</label>
                            <select value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} className="w-full bg-gray-50 border-none rounded-2xl py-4 px-6 text-sm font-black uppercase tracking-widest focus:ring-2 focus:ring-bakery-primary focus:bg-white transition-all appearance-none" required>
                            ...
                            <div className="col-span-2 grid grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-gray-50 border-none rounded-2xl py-4 px-6 text-sm font-black tracking-widest focus:ring-2 focus:ring-bakery-primary focus:bg-white transition-all"
                                        required
                                    />
                                </div>
                                <div className="flex items-end">
                                    <label className={`w-full flex items-center justify-center gap-3 p-4 rounded-2xl border transition-all cursor-pointer ${formData.cakePricing.customizationAvailable ? 'bg-bakery-primary border-bakery-primary text-white' : 'bg-gray-50 border-gray-100 text-gray-600 hover:text-bakery-primary'}`}>
                                        <input
                                            type="checkbox"
                                ...
                                <div className="flex items-end">
                                    <label className="w-full flex items-center justify-center gap-3 p-4 rounded-2xl border transition-all cursor-pointer bg-gray-50 border-gray-100 text-gray-600 hover:text-bakery-primary">
                                        <input
                                            type="checkbox"
                            ...
                            <div className="space-y-4">
                                {(imagePreview || formData.image) && (
                                    <div className="relative w-full h-56 bg-gray-50 rounded-[2rem] overflow-hidden border border-gray-100 group">
                                        ...
                                        <button type="button" onClick={() => { setImageFile(null); setImagePreview(''); setFormData({ ...formData, image: '' }); }} className="absolute top-4 right-4 p-3 bg-red-600 text-white rounded-xl hover:bg-bakery-primary transition-all shadow-xl">
                                            <X className="w-4 h-4" />
                                        </button>
                                    </div>
                                )}
                                ...
                                <label className="flex flex-col items-center justify-center w-full h-40 bg-gray-50 border-2 border-dashed border-gray-200 rounded-[2rem] cursor-pointer hover:bg-gray-100 transition-all group">
                                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                        <Upload className="w-8 h-8 text-gray-300 mb-3 group-hover:text-bakery-primary transition-colors" />
                                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] group-hover:text-bakery-primary">Click to Upload</p>
                        ...
                        <div className="col-span-2 space-y-4">
                            ...
                            <div className="grid grid-cols-1 gap-4">
                                <label className={`flex items-center justify-center gap-3 p-4 rounded-2xl border transition-all cursor-pointer ${formData.isAvailable ? 'bg-bakery-primary border-bakery-primary text-white shadow-xl shadow-bakery-light' : 'bg-gray-50 border-gray-100 text-gray-400 hover:text-bakery-primary'}`}>
                    ...
                    <div className="flex gap-4 pt-6">
                        ...
                        <button type="submit" disabled={loading || uploading} className="flex-1 py-5 bg-bakery-primary text-white rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-bakery-primary disabled:opacity-50 transition-all shadow-xl shadow-bakery-light">
